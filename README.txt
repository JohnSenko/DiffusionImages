There are four folders here with images in them to help demonstrate the functionality developed for image enhancement. This includes algorithms for local contrast enhancement, dynamic range adjusted, and edge preserving smoothing. Each directory has its own own _README* documents.
...
The images in folder "SourceImages" are the input used to generate the results in the other three folders. These are provided so that you might have the ability to apply other image enhancement software to them for comparison. (Note: the are mostly .jpg files, with a few HDR ".exr" format files.)

The other three folders all contain composites of 4 images generated with different setting for ease of comparison.

The amount of dynamic range reduction in these examples has all been done automatically. Hand chosen settings for that and he amount of contrast enhancement will consistently produce more aesthetically pleasing results.
